...
test('Customer successfully purchases a product', async ({ page }) => {
  ...
  await shopPage.searchProduct('Rusty-Bot 101');

  // Step 4: Open the product detail page from the listing.
  await shopPage.openProductByName('Rusty-Bot 101');
  await expect(productPage.title).toContainText('Rusty-Bot 101');

  // Step 5: Add the product to the cart.
  await productPage.addToCart();

  // Step 6: Verify a success indicator (cart badge appears after add + reload).
  ...
  await cartPage.expectItemInCart('Rusty-Bot 101');
